...
def test_generate_columns():
    ...
    assert len(columns) == TOTAL_COLUMNS, f"Expected {TOTAL_COLUMNS} columns, but got {len(columns)}"
    ...
    assert all(col in columns for col in expected_sample), "Generated columns do not match the expected format"
...
def test_get_column_names():
    ...
    assert len(column_names) == TOTAL_COLUMNS, f"Expected {TOTAL_COLUMNS} column names, but got {len(column_names)}"
...
def test_build_training_features():
    ...
    assert sorted(training_features) == sorted(expected_columns), \
        f"Expected columns {expected_columns}, but got {training_features}"
    
    # Ensure no unrelated columns are included
    assert 'unrelated_column' not in training_features, \

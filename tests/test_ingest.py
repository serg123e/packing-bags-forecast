...
def test_extra_features(sample_data):
    sample_data['delivery_time'] = pd.to_datetime(
        sample_data['delivery_time'], utc=True
    )
    ...
    assert (
        enriched_data['day_of_year'].iloc[0]
        == enriched_data['delivery_time'].dt.dayofyear.iloc[0]
    )

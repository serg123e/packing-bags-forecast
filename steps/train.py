...
def train_model(df_filtered, target_column, hub_id, features):
    ...
    if (target_column == "deep_frozen_bags"):
        ...
        params = {'colsample_bytree': 0.8049310664813739, 'gamma': 0.31904734990860323, 'learning_rate': 0.020987723995735754, 
                  'max_depth': 8, 'n_estimators': 1400, 'objective': 'reg:squarederror', 'random_state': 42, 'subsample': 0.6878477231583816, 'verbosity': 1}
        ...
        params = {'colsample_bytree': 0.9332506592696221, 'gamma': 0.33126595740822656, 'learning_rate': 0.011121117890656158, 'max_depth': 8, 'n_estimators': 700, 
       'objective': 'reg:squarederror', 'random_state': 42, 'subsample': 0.5581688088227714, 'verbosity': 1}
...
def main():
    ...
    df_filtered = data.dropna(subset=['bags_used', 'cold_bags_used', 'deep_frozen_bags_used'])
    ...
    for hub_id in [1,4]:
        ...
        for target_column in target_columns:
            ...
            output_file_path = os.path.join(EFS_MOUNT_POINT, 'models', f"{target_column}_{hub_id}.joblib")
            joblib.dump(model, output_file_path, compress=True)
            result.append(f"Model for prediction of {target_column} of hub {hub_id} trained and saved to {output_file_path}")
